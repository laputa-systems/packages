...
proc main(root: Path = /rootfs) [fs, process, error] {
  ...
  proof.ensure(fs.exists(fp"${root}/usr/sbin/wpa_supplicant")?, "wpa_supplicant", "missing wpa_supplicant binary")?
  proof.ensure(fs.exists(fp"${root}/usr/sbin/wpa_cli")?, "wpa_supplicant", "missing wpa_cli binary")?
  proof.ensure(fs.exists(fp"${root}/usr/sbin/wpa_passphrase")?, "wpa_supplicant", "missing wpa_passphrase binary")?
  ...
  let status = process.run(process.command_argv(
    fp"${root}/usr/sbin/wpa_supplicant",
    ["wpa_supplicant", "--help"],
    /,
    {},
  ))?
